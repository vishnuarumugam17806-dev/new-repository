...
class SharedDatabase(BaseModel):
    ...
    # Core outputs
    database_type    = db.Column(db.String(50), default='sqlserver')   # sqlserver | mysql | oracle | mongodb | excel
    ...
    sample_data      = db.Column(db.Text, nullable=True)   # SQL INSERT statements or sample docs/rows
    ...
    def __repr__(self):
        return f'<Schema {self.project_name} ({self.database_type})>'
...
class SchemaVersion(BaseModel):
    ...
    sql_code       = db.Column(db.Text, nullable=False)
    change_summary = db.Column(db.String(255), default='Schema updated')
    created_at     = db.Column(db.DateTime, default=_utcnow)
...
class Rating(BaseModel):
    ...
    created_at = db.Column(db.DateTime, default=_utcnow)
    __table_args__ = (db.UniqueConstraint('user_id', 'schema_id', name='unique_user_schema_rating'),)
...
# ── Database Instances / Connections ──────────────────────────────────────────
class DatabaseInstance(BaseModel):
    __tablename__  = 'database_instances'
    id             = db.Column(db.Integer, primary_key=True)
    user_id        = db.Column(db.Integer, db.ForeignKey('users.id'), nullable=True)
    name           = db.Column(db.String(100), nullable=False)
    db_type        = db.Column(db.String(50), nullable=False)   # sqlserver | mysql | oracle | mongodb | excel
    host           = db.Column(db.String(255), default='localhost')
    port           = db.Column(db.Integer, nullable=True)
    username       = db.Column(db.String(100), default='')
    encrypted_pwd  = db.Column(db.String(255), default='')   # Never plain text
    database_name  = db.Column(db.String(100), default='')
    connection_uri = db.Column(db.Text, default='')
    extra_params   = db.Column(db.Text, default='{}')
    is_active      = db.Column(db.Boolean, default=True)
    created_at     = db.Column(db.DateTime, default=_utcnow)

    def __repr__(self):
        return f'<DatabaseInstance {self.name} ({self.db_type})>'
...
class DownloadLog(BaseModel):
    ...
    file_type     = db.Column(db.String(20), default='sql')   # sql | sqlite | nosql | excel
